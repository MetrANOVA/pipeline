...
class IFMIBInterfaceTrafficProcessor(BaseInterfaceTrafficProcessor):
    def __init__(self, pipeline):
        ...
        self.ifname_lookup_table = os.getenv(
            "TELEGRAF_IFMIB_IFNAME_LOOKUP_TABLE", "ifindex_to_ifname"
        )
        self.match_fields = [
            ["name"],
            ["timestamp"],
    ...
    def build_message(self, value: dict, msg_metadata: dict) -> list[dict]:
        ...
        if interface_name is None:
            self.logger.debug(
                f"Interface name not found for device {device} and oidIndex {oid_index}"
            )
            return None
        # Build interface_id and interface_ref
        interface_id = f"{device}::{interface_name}"
        interface_ref = self.pipeline.cacher("redis").lookup(
            "meta_interface", interface_id
        )
        ...
        for target_name, telegraf_mapping in self.metric_fields.items():
            formatted_record[target_name] = value.get("fields", {}).get(
                telegraf_mapping["field"], None
            )
            if "scale" in telegraf_mapping:
                formatted_record[target_name] = self.scale_value(
                    formatted_record[target_name], telegraf_mapping["scale"]
                )
        ...
        if formatted_record["admin_status"] in admin_status_map:
            formatted_record["admin_status"] = admin_status_map[
                formatted_record["admin_status"]
            ]
        oper_status_map = {
            1: "up",
            2: "down",
        ...
        if formatted_record["oper_status"] in oper_status_map:
            formatted_record["oper_status"] = oper_status_map[
                formatted_record["oper_status"]
            ]
...
class DataGenericMetricProcessor(BaseDataGenericMetricProcessor):
    def __init__(self, pipeline):
        super().__init__(pipeline)
        self.logger = logger
        self.required_fields = [["timestamp"], ["fields"], ["tags"]]
    ...
    def match_message(self, value):
        ...
        if (
            self.resource_type_map.get(self.rules[val_name]["resource_type"], None)
            is None
        ):
            return False
    ...
    def format_value(
        self, value: str, format_type: str | None, format_opts: dict | None, ext: dict
    ) -> str:
        ...
        for rid_def in resource_id_rules:
            if rid_def.get("type") == "field":
                ...
                if current is not None:
                    ...
                    current = self.format_value(
                        str(current), format_type, rid_def.get("format_opts", None), ext
                    )
                    lookup_table = rid_def.get("lookup", None)
                    current = self.lookup_value(
                        current, resource_id_parts, lookup_table
                    )
    ...
    def build_message(self, value, msg_metadata):
        ...
        if resource_type is None:
            self.logger.debug(
                "No resource_type defined in rule, cannot determine table name"
            )
            return []
        ...
        for field, field_value in value.get("fields", {}).items():
            ...
            if table_name not in self.table_name_map:
                self.logger.debug(
                    f"Table name {table_name} not recognized, skipping field {field}"
                )
                continue
