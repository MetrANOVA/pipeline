...
class LookupTableProcessor(BaseRedisProcessor):
    ...
    def __init__(self, pipeline):
        ...
        self.expires = int(
            os.getenv("REDIS_TELEGRAF_LOOKUP_TABLE_EXPIRES", "86400")
        )  # default 1 day
    ...
    def format_value(self, value: str, format_type: str | None) -> str:
        ...
        # Use hostname formatter for any format name
        # This includes 'short_hostname' and any custom formats
            ...
            for key_config in key_configs:
                if key_config.get("type") == "field":
                    ...
                    if field_value is not None:
                        key_parts.append(
                            self.format_value(
                                str(field_value), key_config.get("format", None)
                            )
                        )
                    else:
                ...
                if lookup_value is not None:
                    ...
                    formatted_record = {
                        ...
                        "value": self.format_value(
                            str(lookup_value), value_config.get("format", None)
                        ),
